...
// Bloquear uma data (quando agendar)
export async function blockDate(date: string, reason?: string): Promise<boolean> {
  const { error } = await supabase
    .from('blocked_dates')
    .upsert({ date, reason: reason || 'Agendamento realizado' }, { onConflict: 'date' })
...
// Desbloquear uma data (quando cancelar)
...
export async function hasBookingsOnDate(date: string): Promise<boolean> {
  const { data, error } = await supabase
    .from('bookings')
    .select('id')
    .eq('date', date)
  ...
  if (error) {
    console.error('Erro ao verificar agendamentos na data:', error)
    return true // Em caso de erro, assume que tem agendamentos para não desbloquear
  }
  return data && data.length > 0
}
